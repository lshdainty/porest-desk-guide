pipeline {
    ...
    environment {
        REPO_URL = "https://github.com/lshdainty/porest-desk-guide.git"
        IMAGE_NAME = "porest-desk-guide"
        SRC_DIR = "${env.POREST_BASE_DIR}/src/desk-guide"
        CONTAINER_NAME = "desk-guide"
    }
    stages {
        stage('Validate') {
            steps {
        ...
        stage('Deploy to Dev') {
            ...
            steps {
                echo "Deploying Desk Guide to Development..."
                sh """
        ...
        stage('Deploy to Prod') {
            when { expression { params.DEPLOY_ENV == 'prod' } }
            steps {
                echo "Deploying Desk Guide to Production..."
                sh """
