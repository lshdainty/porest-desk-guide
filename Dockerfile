...
# 빌드 결과물 복사 — baseUrl 이 /guide/ 라 컨테이너 안 경로도 그대로 맞춘다
COPY --from=builder /app/build /usr/share/nginx/html/guide

EXPOSE 8080

HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 \
    CMD wget -qO- http://127.0.0.1:8080/guide/ >/dev/null || exit 1
